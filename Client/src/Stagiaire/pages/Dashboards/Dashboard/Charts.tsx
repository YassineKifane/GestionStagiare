...
const TrafficResourcesChart = ({ chartId, tasksPercentage }: any) => {
    const chartColors = useChartColors(chartId);

    // Extracting data from taskStatus object
    const todoTasks = tasksPercentage?.todo?.toFixed(2) || 0;
    const inProgressTasks = tasksPercentage?.enCours?.toFixed(2) || 0;
    const completedTasks = tasksPercentage?.termine?.toFixed(2) || 0;

    // Traffic Resources Chart
    const series = [todoTasks, inProgressTasks, completedTasks];
    const options: any = {
        chart: {
            height: 222,
            type: 'radialBar',
                    ...
                    total: {
                        ...
                        formatter: function (w: any) {
                            const total = w.globals.seriesTotals.reduce((a: any, b: any) => {
                                // Convert b to a number if it's not NaN, otherwise return a
                                return isNaN(b) ? a : a + Number(b);
                            }, 0);
                            // Apply toFixed(2) to the total
