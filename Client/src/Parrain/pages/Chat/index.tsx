...
import {POST_USER_MESSAGE, GET_USER_CHAT} from "services/chatApis/chatApis";
...
const Chat = () => {
  ...
  useEffect(() => {
    if (parrainId) {
      socketRef.current.emit("new-user-add", parrainId);
  ...
  const fetchChats = async (userId: any) => {
    console.log("making request rquests to : ", GET_USER_CHAT + userId)
    try {
  ...
  const createMessage = async (chatId: any, senderId: any, text: any) => {
    try {
      ...
      return response;
      // You may want to fetch messages again after creating a new message
    } catch (error) {
  ...
  const [sendedMssg, setSendedMssg] = useState<any>(null);
  ...
  const addMessage = async () => {
    if (curMessage !== "") {
      ...
      socketRef.current.emit("send-message", {
        ...
        text: curMessage,
        senderId: parrainId,
        receiverId: currentIntern.internId,
      });
    }
    setcurMessage("");
  };
  ...
  const userChatOpen = (
    ...
    // Concatenate ReceiveMessage to the existing messages array
  ...
  const checkOnlineStatus = (chat: any) => {
    const chatMember = chat.members.find((member: any) => member !== parrainId);
                ...
                <div>
                  ...
                  <div className="mt-5">
                    ...
                    <h6 className="font-medium">
                      <AlarmClock className="inline-block size-4 ltr:mr-1 rtl:ml-1 text-slate-500 dark:text-zink-200" />{" "}
                      <span className="align-middle">
                        {currentIntern.StageDuration}
                      </span>
                    </h6>
                  </div>
                </div>
