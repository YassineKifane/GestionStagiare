...
const Cvtheque = () => {
  ...
  const deleteInternById = async (internId: string) => {
    try {
        console.log(`Attempting to delete intern with ID: ${internId}`);
        const response = await fetch(`${DELETE_INTERN + internId}`, {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${getToken()}`, // Include the token in the Authorization header
            },
        ...
        if (!response.ok) {
            ...
            console.error(`Failed to delete intern. Status: ${response.status}, Message: ${errorMessage}`);
            throw new Error(errorMessage || 'Failed to delete intern');
        }

        console.log('Intern deleted successfully');
        toast.success('Intern deleted successfully', { autoClose: 2000 });

        // Additional operations after successful deletion
        await deleteImageByUserId(internId);
