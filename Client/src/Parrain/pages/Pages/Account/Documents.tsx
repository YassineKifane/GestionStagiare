...
const Documents = ({parrainId, internId, documents, getAllDocumentsReceived, getAllDocumentsSended, docsReceived}:any) => {
    ...
    //         try {
    //             await Promise.all([
    //                 getAllDocumentsByReceiverId(internId, parrainId, setDocsReceived),
    //                 getAllDocumentsByReceiverId(parrainId, internId, setDocuments)
    //             ]);
        ...
        <React.Fragment>
            ...
            <div className="overflow-x-auto">
                <table className="w-full align-middle border-separate whitespace-nowrap border-spacing-y-1">
                    ...
                    <tbody>
                        {(docsReceived || [])?.map((item: any, index: number) => (
                            <tr key={index} className="bg-white dark:bg-zink-700">
                                <td className="px-3.5 py-2.5 border-y border-transparent">
                                    <div className="flex items-center h-full">
                                        <input
             ...
             <Modal show={show} onHide={toggle} id="defaultModal" modal-center="true"
                ...
                <Modal.Header className="flex items-center justify-between p-5 border-b dark:border-zink-500"
                    ...
                    <Modal.Title className="text-16">{!!isEdit ? "Edit Document" : "Send Document"}</Modal.Title>
                </Modal.Header>
